...
from gef_analyzr.prompts.default_prompts import (
    BEGIN_SYS,
    END_SYS,
    BEGIN_INST,
    END_INST,
...
class ContextSearcher:
    def __init__(self, llm, embedding_model, threshold=0.5):
        ...
        self.question_prompt = PromptTemplate(
            template=templateStr, input_variables=["context", "issue"]
        )

        combine_template = f"{BEGIN_INST} {BEGIN_SYS} {CONTEXT_SYSTEM} {END_SYS} {CONTEXT_COMBINE_INSTRUCTION} {END_INST}"
        self.combine_prompt = PromptTemplate(
            template=combine_template, input_variables=["issue", "summaries"]
        )
    ...
    def qa_search(self, query):
        docs = self.docsearch.get_relevant_documents(
            query
        )  # TODO replace with custom retrieval in database_interface
