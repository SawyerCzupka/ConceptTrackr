...
class ProjectQA:
    def __init__(
        self, vectorstore: VectorStore, llm: LLM, chain_type: Literal["refine"] | None
    ):
        self._database = vectorstore
        self._llm = llm

        chain_mapping = {"refine": self._create_refine_chain}

        self._qa: BaseCombineDocumentsChain = chain_mapping[chain_type]()

    def ask_question(self, question: str, projectID: int) -> str:
    ...
    def _get_documents_from_project(
        ...
        return self._database.search(
            query=query, search_type=search_type, filter=search_filter, **kwargs
        )

    def _create_refine_chain(self):
        question_tmpl = f"{BEGIN_INST} {BEGIN_SYS} {QA_SYSTEM} {END_SYS} {QA_REFINE_INIT} {END_INST}"
        question_prompt = PromptTemplate(
            input_variables=["question", "context_str"], template=question_tmpl
        )

        refine_tmpl = (
            f"{BEGIN_INST} {BEGIN_SYS} {QA_SYSTEM} {END_SYS} {QA_REFINE} {END_INST}"
        )
